...
type meeting =
  | Cross
  | Terminate
  | End; // does a crossing span meet the main span by Crossing it, or by terminating at it?
...
let classifyH: (int, list(span)) => list(spanSummary) =
  (y, spans) =>
    List.map(
      s =>
        {
          m: s.p1.yi == y || s.p2.yi == y ? Terminate : Cross,
          pos: s.p1.xi,
        },
      spans,
    );

let rec convertHelper:
  (float, int, int, int, list(spanSummary), int => float) =>
  list(feature) =
  (thickness, ss, sf, n, meets, gToWH) => {
    let t = thickness;
    switch (meets) {
...
let enhance: (span, list(spanSummary)) => list(spanSummary) =
  (s, flist) => {
    switch (flist) {
    ...
    | [head, ..._tail] =>
      ...
      let fixedLast =
        if (lastFeature.pos != s.p2.xi) {
          flist @ [{m: End, pos: s.p2.xi}];
        } else {
          flist;
        };
      if (head.pos != s.p1.xi) {
        [{m: End, pos: s.p1.xi}, ...fixedLast];
      } else {
        fixedLast;
      };
    };
...
let rec cleanup: list(feature) => list(feature) =
  ss => {
    switch (ss) {
    | [] => []
    | [q] => [q]
...
let convertOne: (span, list(span), settings) => list(feature) =
  (s, spans, settings) => {
...
let flipSpan: span => span =
  s => polish({p1: flipPoint(s.p1), p2: flipPoint(s.p2)});
let flipSpanList: list(span) => list(span) =
  s => List.map(flipSpan, s);
let flipFeature: feature => feature =
  q => {
    switch (q) {
    | G(_)
    | S(_)
...
let prepAndConvert:
  (list(span), list(span), settings) => list(list(feature)) =
  (hSpans, vSpans, settings) => {
    let qq: span => list(feature) =
      s => {
        let crosses = List.filter(v => meets(s, v), vSpans);
        let cross2 =
          List.sort(
            (s1, s2) => compare(s1.p1.xi, s2.p1.xi),
            crosses,
          );
...
let convertAll: (list(span), settings) => list(list(feature)) =
  (drawing, settings) => {
...
let convertAll2:
  (list(span), settings) =>
  (list(list(feature)), list(list(feature))) =
  (drawing, s) => {
    let d = List.map(polish, drawing); //make sure spans are in increasing order
    let dH = List.filter(isHorizontal, d);
    let dV = List.filter(x => !isHorizontal(x), d);
    ...
    let sFlip = {
      ...
      width: s.depth,
      depth: s.width,
      dipPercentageH: s.dipPercentageV,
      dipPercentageV: s.dipPercentageH,
    };
    let fV =
      flipFeatureListList(
        prepAndConvert(flipSpanList(dV), flipSpanList(dH), sFlip),
      );
    (fH, fV);
  };
...
let string_of_feature: feature => string =
  q =>
    switch (q) {
    | G(f) => "G(" ++ sof(f) ++ ")"
    | S(f) => "S(" ++ sof(f) ++ ")"
...
//type direction = H | V;
let string_of_direction: direction => string =
  q =>
    switch (q) {
    | H => "H"
    | V => "V"
    };
let string_of_meeting: meeting => string =
  q =>
    switch (q) {
    | Cross => "Cross"
    | Terminate => "Term"
...
let string_of_span_summaryi: spanSummary => string =
  ({m: meet, pos: n}) =>
...
let spanList_of_strokeList: list(stroke) => list(span) =
  sl => List.map(s => s.sp, sl);

let converter: (list(stroke), settings) => list(list(feature)) =
  (sl, settings) =>
    convertAll(spanList_of_strokeList(sl), settings);

let stringOfStrokeList: (list(stroke), settings) => string =
  (sl, settings) => {
    let c: list(list(feature)) = converter(sl, settings);
    let make_string = sl => {
      List.fold_right(
        (x, s) => (string_of_feature(x) ++ ", ") ++ s,
        sl,
        "",
      );
    };
...
let dividerToString: divider => string =
  d => {
...
let drawingToDivider: (Types.state, drawing) => divider =
  (st, drawing) => {
...
let rec allBoxesHelper:
  (panelGeom, specs, float, bool) => list(boxSpec) =
  (panel, sp, x, isHorizontal) =>
    ...
    | [G(f), ...rst] =>
      dipGap(
        ...
        f,
        sp.height,
        isHorizontal ? sp.dipSizeH : sp.dipSizeV,
      )
      @ allBoxesHelper(rst, sp, x +. f, isHorizontal)
    //allBoxesHelper(rst, sp, x +. f, isHorizontal)
    | [D(f), ...rst] =>
...
let allBoxes: (panelGeom, specs, bool) => list(boxSpec) =
  (panel, sp, isHorizontal) => [
    outerBox(panel, sp),
    ...allBoxesHelper(panel, sp, 0.0, isHorizontal),
  ];
...
let rec boxListToPathD: (list(boxSpec), specs, float) => string =
  (bsl, sp, dipSize) => {
    ...
    | [] => ""
    | [b, ...tail] =>
      boxToPathD(b, sp, dipSize)
      ++ boxListToPathD(tail, sp, dipSize)
    };
  };
...
/* inputs:
        x,y: SVG location to place a piece of text in green
        s: the text string
      output: an SVG element that puts the text at the location
   */
...
/*
  inputs:
    panel: a description of a panel
    specs: ???
    x, y: the location of the drawing within the SVG coordinate system
    dipSize: how much the panel "dips" in the middle to make grabbing things easy
    Output: svg for cutting that panel on a laser cutter, as an SVG 'path'
 */
...
/*
 input: a divider assembly
 Output: svg for cutting that divider on a laser cutter, with red for cutting, blue for visualization, and green for panel-labeling
 */
let dividerToSVG: divider => string =
  dr => {
    ...
    let windowHeight =
      int_of_float(
        40.0
        +. float_of_int(panelCount)
        *. 72.0
        *. (dr.spec.height +. 0.125),
      );
    ...
    let offset =
      (float_of_int(List.length(dr.horiz)) +. 0.2)
      *. 72.0
      *. (dr.spec.height +. 0.125);
    let panelPathsV =
      List.mapi(
