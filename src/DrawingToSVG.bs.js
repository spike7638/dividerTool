...
import * as Caml_primitive from "bs-platform/lib/es6/caml_primitive.mjs";
...
function prepAndConvert(hSpans, vSpans, settings) {
  var qq = function (s) {
    ...
    var cross2 = List.sort((function (s1, s2) {
            return Caml_primitive.caml_int_compare(s1.p1.xi, s2.p1.xi);
          }), crosses);
...
function convertAll2(drawing, s) {
  ...
  var sFlip_counter = s.counter;
  var sFlip_thickness = s.thickness;
  var sFlip_width = s.depth;
  var sFlip_depth = s.width;
  var sFlip_height = s.height;
  var sFlip_spacing = s.spacing;
  var sFlip_includeEnclosure = s.includeEnclosure;
  var sFlip_dipPercentageH = s.dipPercentageV;
  var sFlip_dipPercentageV = s.dipPercentageH;
  var sFlip = {
    counter: sFlip_counter,
    thickness: sFlip_thickness,
    width: sFlip_width,
    depth: sFlip_depth,
    height: sFlip_height,
    spacing: sFlip_spacing,
    includeEnclosure: sFlip_includeEnclosure,
    dipPercentageH: sFlip_dipPercentageH,
    dipPercentageV: sFlip_dipPercentageV
  };
  var fV = List.map(flipFeatureList, prepAndConvert(List.map(flipSpan, dV), List.map(flipSpan, dH), sFlip));
  return [
          fH,
          fV
        ];
